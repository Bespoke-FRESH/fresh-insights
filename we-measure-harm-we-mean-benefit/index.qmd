...
**The consequence.** Of the organizations sitting between evidence and action,
one — [MDCalc](https://www.mdcalc.com/) — produces a decision artifact you could
...
[^provenance]: Stated plainly because it matters for an essay about unsupported claims. The observation comes from applying written stage-inclusion tests by hand to the organizations in the survey — under those tests, the entities I had coded into a *decide* stage collapse from seven to one, because six of them are primary-care businesses, and a clinic is where deciding happens rather than a decision instrument. The tests were written after the current release shipped, so the [published map](../ai-for-science-map/) does not yet reflect them; MDCalc is not in it. The next release applies them. Until then the claim rests on the reasoning above, not on the artifact, and I would rather say so than link you to something that does not support it.
