...
CREATE TABLE IF NOT EXISTS engine_log (
  id         INTEGER PRIMARY KEY AUTOINCREMENT,
  path       TEXT NOT NULL,             -- route SHAPE, not instance: /intake/:id/score, not
                                         -- /intake/abc123/score (see redactRouteShape in
                                         -- index.js) — a raw id here would let same-day rows
                                         -- cluster which records one device touched
  ip_hash    TEXT,
